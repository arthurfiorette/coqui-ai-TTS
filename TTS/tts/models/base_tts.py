...
class BaseTTS(CloningMixin, BaseTrainerModel):
    ...
    def get_aux_input_from_test_sentences(self, sentence_info: list[str] | list[list[str]]) -> dict[str, Any]:
        if hasattr(self.config, "model_args"):
        ...
        if speaker_name is None and self.speaker_manager is not None:
            speaker_name = random.sample(self.speaker_manager.speaker_names, 1)[0]
    ...
    def _get_speaker_id_or_dvector(
        ...
        if self.speaker_manager is not None:
            ...
            if speaker is not None:
                if self.config.use_d_vector_file and speaker in self.speaker_manager.embedding_names:
                ...
                elif self.config.use_speaker_embedding and speaker in self.speaker_manager.name_to_id:
